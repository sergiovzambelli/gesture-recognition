...
MODEL_SAVE_PATH = "best_gesture_rnn_assets\\model.pth"
...
MAX_NUM_HANDS = 2
LANDMARK_DIM = MAX_NUM_HANDS * NUM_LANDMARKS_PER_HAND * \
    2  # (x,y) per ogni landmark
...
class GestureRNNModel(nn.Module):
    def __init__(
        ...
        self.rnn = nn.LSTM(
            ...
            # L'uso di un LSTM bidirezionale può catturare dipendenze temporali in entrambe le direzioni
            bidirectional=True,
        )
...
def train(model, train_loader, val_loader, optimizer, criterion, epochs, patience):
    ...
    for epoch in range(epochs):
        ...
        play_sound("assets\\VAI_UOMO.mp3")
        ...
        if val_loss < best_val_loss:
            ...
            print(
                f"   ✅ Modello salvato: Val Loss migliorata a {best_val_loss:.4f}")
        else:
            epochs_no_improve += 1
...
if __name__ == "__main__":
    ...
    if df.empty:
        ...
        print(
            f"Pesi calcolati per la loss function: {class_weights.cpu().numpy()}")
        ...
        # Creazione Datasets
        train_dataset = GestureLandmarkDataset(
            train_df, label_map, augment=True)
        val_dataset = GestureLandmarkDataset(val_df, label_map, augment=False)
        test_dataset = GestureLandmarkDataset(
            test_df, label_map, augment=False)
        ...
        torch.save(model.state_dict(), "final_gesture_rnn_assets\\model.pth")
        print("\nPesi finali del modello salvati in 'final_gesture_rnn_assets\\model.pth'.")
